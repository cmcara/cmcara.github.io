...
<div style="display: flex; flex-direction: column; gap: 20px; align-items: center;">
  ...
  <div style="width: 100%; height: 520px; overflow: hidden; border: none;">
...
Indoor LiDAR scanning uses advanced laser and sensor technology to create highly accurate ‘digital twins’ of built environments from point clouds captured as reflectance data. Mobile scanning leverages Self Localization and Mapping [(SLAM)](https://www.mathworks.com/discovery/slam.html) algorithms to capture data continuously via backpack mounted sensors which is often combined with panorama photos at regular intervals and more traditional static scanners on tripods to develop maps, models, and virtual walkthroughs. Digital twins produced by companies like [Prologue Hospitality](https://prologuehospitality.com/) can be incredibly helpful for major hospitality and infrastructure developments as designers, engineers, and architects can view the location in high resolution without travelling on-site. Unfortunately, turning captured data into actionable models requires a number of occasionally tedious and manual processing steps to clean up the point cloud data.
...
I first down sample the point cloud to voxels to help with initial visualization and classification before implementing Random Sampling Consensus [(RANSAC)](https://en.wikipedia.org/wiki/Random_sample_consensus) to identify the major planes as walls or floors. Principal Component Analysis [(PCA)](https://en.wikipedia.org/wiki/Principal_component_analysis) helps analyze each plane independently in 2D and classify gaps as either doors or gaps to be filled in (windows, mirrors, etc.). Improving the door detection algorithm through further testing or machine learning classification would likely improve the output overall but is a future endeavor. I borrow from [Ray tracing](https://en.wikipedia.org/wiki/Ray_tracing_(graphics)) and calculate the shortest distance from the measured points to the trajectory line that connects all of the photograph locations. At the voxel level, seeing where these rays cross established and filled-in wall planes gives a rough estimate for ghost points. Losing resolution defeats the purpose of using LiDAR in the first place though, so the next step translates the ‘ghost’ classification from the voxel level to the full point cloud using [K-Dimensional Trees](https://en.wikipedia.org/wiki/K-d_tree). A weakness to the current logic occurs when I have long hallways where the scanner collects data on either side of the wall but only travels down one side. This ‘tricks’ the workflow into thinking real data was captured through a wall. This is happening in the plot above where the closest point on the scanner trajectory to the far hallway is actually in the bedroom, which necessitates capturing through a wall. Dropping walls sufficiently far from the scanner trajectory helps alleviate this in other clouds but is less for this particular example. Static scanners have fixed origin points tied to each measurement which should alleviate this specific issue, but I have not built out static functionality yet. 
...
<div style="width: 850px; height: 630px;">
...
For a past Earth Analytics assignment, I gathered vegetation data from [NAIP](https://naip-usdaonline.hub.arcgis.com/) satellite imagery for NDVI values and various measures, the [CDC Places](https://www.cdc.gov/places/index.html) dataset on asthma rates at the census tract level, and income data from the 5-Year American Community Survey from the [Census Bureau](https://www.census.gov/data/developers/data-sets/acs-5year.html), in an attempt to model a relationship between greenspace, income, and asthma rates. The original OLS regression I ran had a number of structural issues and assumption violations but easily let me test the data in practice. For more recent assignments in Statistical Methods and Deep Learning, I improved the model and process in a variety of ways using agentic coding tools to implement a generalized linear model instead. By feeding the original workbook into two separate models, (cloud-based Gemini 3 and local DeepSeek-R1-Distill-Qwen-14B), I was able to compare and contrast suggestions to fine tune improvements to the model. While my hope was to see improvements in the predictive power, my primary goal was to evaluate the effectiveness of current open-source agentic tools in bridging the gap between subject matter expertise and statistical rigor/application. In a general sense, many earth scientists only known the statistical models previously established in their fields, while applied mathematicians are often limited to the fields that fit their models nicely. From my experiences, and those of many others, leveraging agentic tools while maintain a human element with some adjacent experience is a very helpful way to learn and implement new techniques to technical workflows. [Read more about the process here.](projects/greenspace_portfolio_upd.html)
